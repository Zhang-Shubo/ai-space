...
- `docs/` - project documentation and diagrams. `docs/architecture.svg` is the architecture figure used in the README. Service designs: `scheduler.md`, `storage.md`, `notify.md` (chat notifications, design only), `panel.md` (web UI, chat, widgets), `peers.md` (one panel over several machines), `i18n.md` (languages of the panel and the manifest's `i18n:` section), `app-spec.md` (the app contract), and `install.md` (first install on a server).
